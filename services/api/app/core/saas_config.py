...
class SaaSConfig(BaseSettings):
    ...
    USAGE_RESET_DAY: int = Field(
        1, env="USAGE_RESET_DAY"
    )  # Day of month to reset usage
    USAGE_ALERT_THRESHOLD: float = Field(
        0.8, env="USAGE_ALERT_THRESHOLD"
    )  # 80% of limit
    ...
    BRANDING_CUSTOMIZATION_ENABLED: bool = Field(
        True, env="BRANDING_CUSTOMIZATION_ENABLED"
    )
    ...
    def get_ai_request_cost(self, model: str, token_count: int) -> Decimal:
        ...
        base_price = self.AI_REQUEST_PRICING.get(
            model, self.AI_REQUEST_PRICING["custom-model"]
        )
    ...
    def get_workflow_execution_cost(self, complexity: str) -> Decimal:
        ...
        return self.WORKFLOW_EXECUTION_PRICING.get(
            complexity, self.WORKFLOW_EXECUTION_PRICING["basic"]
        )
    ...
    def get_plan_limits(self, plan_id: str) -> Dict[str, int]:
        ...
        return {
            ...
            "max_ai_requests_per_month": features.get(
                "max_ai_requests_per_month", 10000
            ),
