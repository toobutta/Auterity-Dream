...
### 2. Advanced Debug Action
```yaml
# .github/copilot/actions/comprehensive-debug.yml
...
## Debug Action Examples

### 1. Error Analysis Action
```javascript
...
async function analyzeError(errorMessage, filePath) {
  ...
  return {
    error: errorMessage,
    suggestions,
    relatedCode: lines.slice(Math.max(0, errorLine - 5), errorLine + 5)
  };
}
...
async function debugTests(testFile) {
  return new Promise((resolve, reject) => {
    exec(`npm test -- --testPathPattern=${testFile} --verbose`, (error, stdout, stderr) => {
      if (error) {
        // Parse test failures
...
```yaml
# .github/workflows/debug-on-issue.yml
name: Debug on Issue
on:
  issues:
...
async function handleSlackDebug(slackMessage) {
  ...
  const codeMatch = slackMessage.text.match(/```[\s\S]*?```/);
  if (!codeMatch) return;
...
### 1. Multi-Step Debug Pipeline
```yaml
# .github/copilot/actions/multi-step-debug.yml
...
# Avoid generic names
name: "Debug"  # Too vague
triggers:
  - "debug"    # Conflicts with other actions
```
...
copilot-actions debug --type typescript

# Run with custom parameters
copilot-actions analyze --file src/app.ts --rules eslint,recommended

# Run comprehensive debug suite
copilot-actions debug --comprehensive --output json
```

## Monitoring and Analytics

### 1. Debug Action Metrics
```javascript
...
function trackSuccess(actionType, success) {
  ...
  const rate = (current.successful / current.total) * 100;
...
<parameter name="filePath">c:\Users\Andrew\OneDrive\Documents\auterity-error-iq\COPILOT_DEBUG_ACTIONS_GUIDE.md
